"""
Robokidy Innovative Centre - Billing Software v5.1 (Bugfix Release)
Fixes in this release:
 - CRITICAL: _ensure_schema() was appending new headers to row 1 without
   verifying column alignment, which silently corrupted column-index lookups
   after repeated app restarts. This caused duplicate Invoice/Receipt numbers
   and a crash when creating invoices (e.g. for custom courses like "Lego")
   once the sheet had ~13+ rows. Fixed: schema migration now only adds
   genuinely missing columns and never re-indexes existing ones.
 - Student ID showing as "...-00" was a column-width display issue only
   (the real ID was always "...-001"); Student ID column width increased
   so the full ID is visible without scrolling.
 - next_student_id / next_inv / next_rcp rewritten to scan ALL existing IDs
   safely (handles blank/garbage values, never collides).
 - Added defensive validation before creating invoices/payments so a bad
   row elsewhere in the sheet can't crash the whole app again.
"""
...
class ExcelStore:
    ...
    def _ensure_schema(self):
        ...
        BUGFIX (v5.1): the previous version did:
            ws.cell(1, ws.max_column+1).value = h
        for every missing header, on EVERY app start. If max_column ever
        drifted (e.g. a stray formatted-but-empty cell existed past the
        last real header — which openpyxl can report after repeated saves),
        new headers were written to the WRONG column index, while old data
        rows kept their original column positions. Every later lookup like
        hdrs.index("Invoice No") then pointed at a different column than
        the one the data was actually written in, producing duplicate IDs,
        wrong invoice totals, and KeyErrors/crashes once enough rows/saves
        had accumulated (matches the ~13-entry crash report).

        Fix: we only ever append a missing header to the FIRST truly empty
        column directly after the last non-empty header cell, and we never
        touch a column that already has a header. We also verify there are
        no duplicate header names (which would alias two different data
        columns onto the same key) and abort with a clear error instead of
        silently corrupting data if that's ever detected.
        """
        ...
        for name,hdrs in [("Students",STUDENT_HEADERS),("Invoices",INVOICE_HEADERS),
            ...
            ws=wb[name]
            # Determine the real last header column (first cell with a value,
            # scanning from the right, ignoring formatting-only empty cells)
            last_col = 0
            for c in range(ws.max_column, 0, -1):
                if ws.cell(1, c).value not in (None, ""):
                    last_col = c
                    break
            current = [ws.cell(1, c).value for c in range(1, last_col+1)]

            # Guard against duplicate headers corrupting column lookups
            seen = set()
            dupes = [h for h in current if h and (h in seen or seen.add(h))]
            if dupes:
                # Don't touch the sheet — surface the problem instead of
                # silently writing more bad data on top of it.
                raise RuntimeError(
                    f"Sheet '{name}' has duplicate column headers {dupes}. "
                    f"This indicates earlier data corruption. Please restore "
                    f"from a backup of robokidy_billing_data.xlsx, or open the "
                    f"file in Excel and remove the duplicate header column(s) "
                    f"by hand before continuing.")

            missing = [h for h in hdrs if h not in current]
            for h in missing:
                last_col += 1
                ws.cell(1, last_col).value = h
                changed = True
            if missing:
                self._sh(ws)
    ...
    def _sh(ws):
        ...
        for i,col in enumerate(ws.columns,1):
            ...
            # Student ID column gets extra width so full ID (e.g.
            # RK-WD-L1-2026-001) is always visible without truncation.
            header = ws.cell(1,i).value
            min_w = 24 if header in ("Student ID","Invoice No","Receipt No") else 12
            width=min(max([len(v) for v in values]+[min_w])+2,34)
            ws.column_dimensions[get_column_letter(i)].width=width

    def rows(self,sheet):
        wb=self._wb(); ws=wb[sheet]
        # Only read headers up to the real last non-empty header cell —
        # avoids picking up stray formatted-but-empty trailing columns.
        last_col = 0
        for c in range(ws.max_column, 0, -1):
            if ws.cell(1, c).value not in (None, ""):
                last_col = c
                break
        hdrs=[ws.cell(1,c).value for c in range(1,last_col+1)]
        out=[]
        for row in ws.iter_rows(min_row=2, max_col=last_col, values_only=True):
    ...
    def _extract_trailing_number(value: str) -> int | None:
        """Safely pull the trailing numeric segment off an ID like
        'RKI-INV-2026-0007' -> 7. Returns None if it doesn't parse —
        never raises, so one bad/garbage row can't break ID generation."""
    ...
    def _next_id(self,sheet,id_col,prefix,fmt="{p}{n:04d}",year=True):
        ...
        nums=[]
        for r in self.rows(sheet):
            v = str(r.get(id_col,"") or "")
            if v.startswith(p):
                n = self._extract_trailing_number(v)
                if n is not None:
                    nums.append(n)
    ...
    def next_student_id(self,course,level):
        code=COURSE_CODES.get(course,(course or "OT")[:2].upper() or "OT")
        lvl=(level or "").replace("Level ","L").replace("All Levels","ALL").replace(" ","").upper() or "L1"
        p=f"RK-{code}-{lvl}-{datetime.now().year}-"
        nums=[]
        for r in self.rows("Students"):
            v = str(r.get("Student ID","") or "")
            if v.startswith(p):
                n = self._extract_trailing_number(v)
                if n is not None:
                    nums.append(n)
    ...
    def add_student(self,d):
        ...
        code=COURSE_CODES.get(d["course"],(d["course"] or "OT")[:2].upper() or "OT")
    ...
    def create_invoice(self,d):
        if not str(d.get("sid") or "").strip():
            raise ValueError("No student selected — pick a student before creating the invoice.")
        if money(d.get("fee")) <= 0:
    ...
    def add_payment(self,d):
        if not self.get_invoice(d.get("inv")):
            raise ValueError(f"Invoice {d.get('inv')} not found — cannot record payment.")
        if money(d.get("paid")) <= 0:
...
class StudentDlg(Dialog):
    ...
    def _save(self):
        ...
        if not self.co.get(): return messagebox.showerror("Missing","Course required",parent=self)
        if not self.lv.get(): return messagebox.showerror("Missing","Level required",parent=self)
...
class PlanDlg(Dialog):
    ...
    def _save(self):
        ...
        try:
            if money(self.fe.get()) <= 0:
                return messagebox.showerror("Missing","Fee must be greater than 0",parent=self)
        except Exception:
            return messagebox.showerror("Invalid","Fee must be a number",parent=self)
...
class BillingApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Billing Software v5.1")
    ...
    def _build(self):
        ...
        tk.Label(tb,text="Billing v5.1",bg=f"#{PRIMARY}",fg=f"#{SECONDARY}",
    ...
    def _treef(self,parent,cols,h=10):
        ...
        for c in cols:
            t.heading(c,text=c)
            # Give Student ID / Invoice No / Receipt No columns more room
            # so the full code is visible (fixes "...-00" truncation look).
            w = 170 if c in ("Student ID","Invoice No","Receipt No") else 110
            t.column(c,width=w,anchor="center",minwidth=60)
    ...
    def _create_inv(self):
        ...
        except ValueError as e: messagebox.showerror("Invalid Invoice",str(e))
        except Exception as e: messagebox.showerror("Error",f"Could not create invoice:\n{e}")
    ...
    def _add_pay(self):
        ...
        except ValueError as e: messagebox.showerror("Invalid Payment",str(e))
        except Exception as e: messagebox.showerror("Error",f"Could not record payment:\n{e}")
